...
from events.models import User, Event  # Adjust the import based on your models (check this!!)
from planoraAPI.settings import env

from django.utils.timezone import now, timedelta
from events.models import Event, EventNotificationConfig
...
def send_event_reminder_mail(event, recipient_list):    
    ...
    html_content = render_to_string("emails/reminder_email.template.html", context)  # Load template
...
def scan_and_send_event_reminders():
    ...
    for event in upcoming_events:
        recipient_list = list(event.attendees.values_list("email", flat=True))  # Get all attendees' emails
...
def send_event_cancellation_mail(event, recipient_list):
    ...
    context = {
        "attendee_name": "[Attendee Name]",  # Replace dynamically
        "event_name": event.name,
        "event_link": f"https://yourwebsite.com/events",
    }

    html_content = render_to_string("event_cancellation_email_template.html", context)  # Load template
    text_content = strip_tags(html_content)  # Plain text fallback

    email = EmailMultiAlternatives(subject, text_content, "your@email.com", recipient_list)
    email.attach_alternative(html_content, "text/html")  # Attach HTML version
    email.send()
...
def send_event_update_mail(event, recipient_list):
    ...
    html_content = render_to_string("modification_email_template.html", context)  # Load template
    text_content = strip_tags(html_content)  # Plain text fallback
